...
release:
	...
	@python3 -c "from pathlib import Path; import re; p=Path('concordia/__init__.py'); t=p.read_text(encoding='utf-8'); t=re.sub(r'^__version__\\s*=\\s*\\\"[^\\\"]+\\\"', '__version__ = \"$(VERSION)\"', t, flags=re.M); p.write_text(t, encoding='utf-8')"
